...
{
    public static class KernelSourceCenterFinder
    {
        public static string FunctionName = "center_finder";
        ...
        public static string FunctionSource =
            @"
                ...
                kernel void " + FunctionName + @"(   __read_only image2d_t input,
                                                    ...
                                                    int samplePointsLength, uchar sensitivity, uchar match, global uchar* result) 
                {
                    int2 centerCoord = (int2)(get_global_id(0), get_global_id(1));
                    int resultPos = get_global_id(0) + (get_global_id(1) * get_global_size(0));
                    
                    uint maxError = (uint)((float)samplePointsLength - ((float)samplePointsLength * ((float)match / (float)100)));
                    uint error = 0;
                    ...
                    for(int i = 0; i < samplePointsLength; i++)
                    {
                        ...
                        int diff = (int)brigtherPixel.x - (int)darkerPixel.x;

                        if(diff < (128 - sensitivity))
                            error += 1;
