...
namespace ImageProcessLib.OpenCL
{
    public class OCLResize : ObservableProperty
    {
        ...
        readonly string cSource =
            @"
                ...
                kernel void scale( global uchar* input, int istride, int ibpp, 
                                global uchar* output, int ostride, int obpp, double scaleValue) 
                {
                    int ox = get_global_id(0);
                    int oy = get_global_id(1);

                    int ix = (int)((double)ox * ((double)100/scaleValue));
                    int iy = (int)((double)oy * ((double)100/scaleValue));
        ...
        public void Start()
        {
            ...
            computeKernel.SetValueArgument(6, scaleValue);
