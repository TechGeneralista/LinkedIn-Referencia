...
{
    public class MonochromeConvertMonochromeColorDependent : ImageBufferMonochrome
    {
        const string functionName = "monochrome_color_dependent";
        ...
        const string functionSource =
            @"
                ...
                kernel void " + functionName + @"(__read_only image2d_t input, __global uchar* output, int ostride)
                {
                    ...
                    output[oaddr] = convert_uint_rte(0.2126*color.x + 0.7152*color.y + 0.0722*color.z);
                }
            ";
        ...
        public MonochromeConvertMonochromeColorDependent(ComputeAccelerator computeAccelerator) : base(computeAccelerator)
        {
