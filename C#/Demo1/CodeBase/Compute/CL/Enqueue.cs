...
namespace Compute.CL
{
    public class Enqueue : ErrorHandler
    {
        ...
        public void WriteBuffer(BufferImage buffer, IntPtr data) => ThrowExceptionOnError(clEnqueueWriteImage(CommandQueue.Handle, buffer.Handle, CLBool.True, new SizeT[] { 0, 0, 0 }, new SizeT[] { buffer.ImageDesc.Width, buffer.ImageDesc.Height, 1 }, 0, 0, data, 0, null, IntPtr.Zero));
        ...
        public void Copy(BufferImage source, BufferImage destination) => ThrowExceptionOnError(clEnqueueCopyImage(CommandQueue.Handle, source.Handle, destination.Handle, new SizeT[] { 0, 0, 0}, new SizeT[] { 0, 0, 0 }, new SizeT[] { source.ImageDesc.Width, source.ImageDesc.Height , 1}, 0, null, IntPtr.Zero));
        ...
        public void ReadBuffer(BufferImage buffer, IntPtr data) => ThrowExceptionOnError(clEnqueueReadImage(CommandQueue.Handle, buffer.Handle, CLBool.True, new SizeT[] { 0, 0, 0 }, new SizeT[] { buffer.ImageDesc.Width, buffer.ImageDesc.Height, 1 }, 0, 0, data, 0, null, IntPtr.Zero));
    }
}
