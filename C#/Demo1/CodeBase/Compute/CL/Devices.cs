...
namespace Compute.CL
{
    public class Devices : ErrorHandler
    {
        ...
        public Device GetDevice(ComputeDeviceTypePriority priority)
        {
            switch(priority)
            {
                case ComputeDeviceTypePriority.GPUCPU:
                ...
                case ComputeDeviceTypePriority.GPU:
                    foreach (Device device in List)
                    {
                        if (device.Type == CLDeviceType.GPU)
                            return device;
                ...
                case ComputeDeviceTypePriority.CPU:
                    foreach (Device device in List)
                    {
                        if (device.Type == CLDeviceType.CPU)
                            return device;
